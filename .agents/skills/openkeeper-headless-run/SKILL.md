...
OpenKeeper's `ScreenshotAppState` writes captures to:

    ~/.OpenKeeper/SCRSHOTS/Main1.png
...
The `ScreenshotAppState` owner is whoever owns `$HOME`. If the project
runs as root, set `HOME=/home/daytona` (or your test account) so the
capture lands in the right `~/.OpenKeeper/`. Also `chmod -R o+rX DK2`
because the demo folder is frequently root-owned:

```bash
chmod -R o+rX /home/daytona/codebase/DK2
mkdir -p /home/daytona/.OpenKeeper/SCRSHOTS
...
DISPLAY=:99 runuser -u daytona -- bash -c "
  HOME=/home/daytona DISPLAY=:99 nohup java -jar '$JAR' -level Conquest >/tmp/ok.log 2>&1 &
"
```
...
| 2 | `DISPLAY=:99 xdotool key --window <WID> Print` | Triggers JME's `ScreenshotAppState` → `~/.OpenKeeper/SCRSHOTS/Main1.png`. |
...
```bash
cp /tmp/shot.png /home/daytona/codebase/scratch/screenshots/Main_$(date +%s)_<method>.png
identify /home/daytona/codebase/scratch/screenshots/Main_*.png
```
